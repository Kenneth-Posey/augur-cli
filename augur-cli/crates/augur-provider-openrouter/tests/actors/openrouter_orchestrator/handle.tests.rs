...
use augur_domain::task_types::{InstructionPrefix, RepoRoot};
...
#[tokio::test]
async fn handle_reports_queue_unavailable_after_shutdown() {
    ...
    let err = handle
        .query_status()
        .expect_err("query_status should fail after actor exit");
    assert!(
        err.to_string().contains("queue unavailable"),
        "error must mention queue availability"
    );
