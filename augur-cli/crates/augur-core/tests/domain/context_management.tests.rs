...
fn tst_cma_008_stage1_excluded_turn_not_cleared() {
    ...
    snap.turn_pairs[0].metadata.excluded_from_clearing = true.into();
    snap.turn_pairs[0].user_message.is_tool_result = true.into();
    snap.turn_pairs[0].assistant_message.is_tool_result = true.into();
...
fn tst_cma_062_stage1_clears_only_tool_result_body_within_turn() {
    let mut snap = sample_snapshot(SessionType::Main);
    snap.turn_pairs[0].user_message.is_tool_result = true.into();
    snap.turn_pairs[0].assistant_message.is_tool_result = false.into();
    let out = run_stage1_content_clearing(snap.clone(), sample_config());
...
fn tst_cma_010_candidate_class_assigned_once() {
    ...
    let eligible_count = snap
        .turn_pairs
        .iter()
        .filter(|turn| !turn.metadata.protected_recent_window && !turn.metadata.objective_changing)
        .count();
...
fn tst_cma_012_protected_turns_not_dropped() {
    let mut snap = sample_snapshot(SessionType::Main);
    snap.turn_pairs[0].metadata.protected_recent_window = true.into();
    ...
    fn tst_cma_011_property_protected_or_objective_changing_turns_not_dropped(
        ...
        snapshot.turn_pairs = turn_flags
            ...
            .map(|(idx, (protected, objective_changing, class_selector))| {
                ...
                turn.metadata.protected_recent_window = (*protected).into();
                turn.metadata.objective_changing = (*objective_changing).into();
                match class_selector {
                    0 => {
                        turn.user_message.is_tool_result = true.into();
                        turn.assistant_message.is_tool_result = true.into();
                    }
                    1 => turn.user_message.body = String::new().into(),
                    _ => turn.metadata.low_semantic_density = true.into(),
                }
                turn
            })
            .collect();
        ...
        for turn in snapshot.turn_pairs {
            if turn.metadata.protected_recent_window.into() || turn.metadata.objective_changing.into() {
